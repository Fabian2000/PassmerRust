...
pub fn get_sidebar_data(search: String) -> Vec<Section> {
    ...
    let mut filtered_sections: Vec<&Section> = sections.iter().filter(|section| {
        section.section_title.to_lowercase().contains(&search.to_lowercase())
    }).collect();

    filtered_sections.sort_by(|a, b| a.section_title.cmp(&b.section_title));

    let mut filtered_sections: Vec<Section> = filtered_sections.iter().map(|x| (*x).clone()).collect();
